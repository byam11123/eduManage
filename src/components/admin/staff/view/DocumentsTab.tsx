...
export function DocumentsTab({ staff }: DocumentsTabProps) {
    ...
    const fieldMap: Record<string, string> = {
        ...
        'joining': 'joiningReportUrl',
        'photo': 'imageUrl' // Field in StaffFormData / API
    }
        ...
        if (file && activeUpload) {
            try {
                ...
                reader.onloadend = async () => {
                    ...
                    if (field) {
                        const response = await staffService.update(staff.id, { [field]: url })
                        if (response.success) {
    ...
    const documents = [
        { id: '10th', name: '10th Marksheet', type: 'Academic', icon: ImageIcon, exists: !!staff.hsMarksheetUrl || !!uploadedDocs['10th'], url: uploadedDocs['10th'] || staff.hsMarksheetUrl },
        { id: '12th', name: '12th Marksheet', type: 'Academic', icon: ImageIcon, exists: !!staff.hssMarksheetUrl || !!uploadedDocs['12th'], url: uploadedDocs['12th'] || staff.hssMarksheetUrl },
        { 
            id: 'graduation', 
            name: 'Graduation Degree', 
            type: 'Academic', 
            icon: ImageIcon, 
            exists: !!staff.gradDegreeUrl || !!uploadedDocs['graduation'],
            url: uploadedDocs['graduation'] || staff.gradDegreeUrl,
            visible: ['graduation', 'post_graduation'].includes(staff.highestQualification || '')
        },
        { 
            id: 'pg', 
            ...
            type: 'Academic', 
            icon: ImageIcon, 
            exists: !!staff.pgDegreeUrl || !!uploadedDocs['pg'],
            url: uploadedDocs['pg'] || staff.pgDegreeUrl,
            visible: staff.highestQualification === 'post_graduation'
        },
        { id: 'photo', name: 'Staff Photo', type: 'Photo', icon: ImageIcon, exists: !!staff.profileImage || !!uploadedDocs['photo'], url: uploadedDocs['photo'] || staff.profileImage },
        { id: 'appointment', name: 'Appointment Letter', type: 'Official', icon: FileText, exists: !!staff.appointmentLetterUrl || !!uploadedDocs['appointment'], url: uploadedDocs['appointment'] || staff.appointmentLetterUrl },
        { id: 'joining', name: 'Joining Report', type: 'Official', icon: FileText, exists: !!staff.joiningReportUrl || !!uploadedDocs['joining'], url: uploadedDocs['joining'] || staff.joiningReportUrl },
                ...
                <CardContent className="p-10">
                    ...
                    {documents.length === 0 && (
                        <div className="py-20 flex flex-col items-center text-center">
                            ...
                            <p className="text-[10px] text-gray-300 font-bold mt-2 uppercase tracking-tight">Institutional records are not required based on the current profile setup.</p>
                        </div>
                    )}
                </CardContent>
            </Card>
